...
expname='dataFrac-'$dataFrac'/regular-pretrain-ntasks='$auxTasks'-lr.'$lr'-optim.'$optim'-ftbsz.'$ftbsz'-bsz.'$bsz'-bntype.'$bntype'-uselast.True-ftlr.'$ftlr
echo 'Regular Pretraining ' $expname ' and save file is ' $expname'.txt'
python -u main.py -ft-lr $ftlr -ft-batch-sz $ftbsz -batch-sz $bsz -prim-datafrac $dataFrac -lr $lr -num-aux-tasks $auxTasks -mode pretrain -num-runs 3 -optimizer $optim -exp-name $exp_fldr'/agnostic_pretraining/'$expname -use-last-chkpt -train-epochs 200 -patience 200 -bn-type $bntype &> run_logs/$expname'.txt'
tail -n 5 run_logs/$expname'.txt'
